...
export default function AdminDocuments() {
  ...
  async function handleUpload(e) {
    ...
    try {
      ...
      setDocsError(err?.response?.data?.error || err?.message || 'Upload failed')
    } finally {
      setDocsLoading(false)
    }
  }
  ...
  if (user?.role && user.role !== 'admin') {
    ...
    return <div className="text-muted-foreground text-sm">Not authorized.</div>
  }
    ...
    <div className="space-y-5">
      <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
        <div>
          ...
          <p className="text-muted-foreground mt-1">Folders + uploads with admin preview support.</p>
        </div>
        ...
        {foldersLoading ? (
          ...
          <div className="rounded-2xl border border-zinc-200 bg-white p-0 overflow-hidden">
            <div className="px-5 py-2 text-xs font-semibold uppercase tracking-wide text-zinc-500 border-b border-zinc-100">
              {currentFolderId == null ? 'Folders' : `Folders in “${pathStack[pathStack.length - 1]?.name}”`}
            </div>
            ...
            <div className="p-5">
              <div className="space-y-6">
                ...
                {currentFolderId != null ? (
                  <div>
                    <div className="mb-2 text-xs font-semibold uppercase tracking-wide text-zinc-500">Files</div>
                    {docsLoading ? (
                      <div className="text-center text-muted-foreground py-6">Loading files…</div>
                    ) : docsError ? (
      ...
      >
        <form onSubmit={handleCreateFolder} className="space-y-4">
          <p className="text-sm text-zinc-500">
            {currentFolderId == null ? 'Create a folder at the top level.' : `Inside “${pathStack[pathStack.length - 1]?.name}”.`}
          </p>
