          ...
          {loading ? (
            ...
            <div className="space-y-3 md:hidden">
              {filteredTeachers.map((teacher) => (
                <div key={teacher._id} className="rounded-2xl border border-border/70 bg-white p-4 shadow-sm">
                  <div className="flex items-start justify-between gap-3">
                    <div>
                      <p className="font-semibold">{teacher.name}</p>
                      <p className="mt-1 break-all text-sm text-muted-foreground">{teacher.email}</p>
                    </div>
                    <button
                      type="button"
                      onClick={() => handleDelete(teacher._id)}
                      className="inline-flex h-11 w-11 items-center justify-center rounded-xl border border-border text-destructive hover:bg-accent"
                    >
                  ...
                  <div className="mt-3 grid grid-cols-2 gap-3 text-sm">
                    <div className="rounded-xl bg-muted/30 px-3 py-2">
                      <p className="text-xs text-muted-foreground">{t('common.classes')}</p>
                      <p className="mt-1 font-medium">{teacher.classesCount ?? 0}</p>
                    </div>
                    <div className="rounded-xl bg-muted/30 px-3 py-2">
                      <p className="text-xs text-muted-foreground">{t('adminTeachers.teachingHours')}</p>
                      <p className="mt-1 font-medium">
                        {teacher.teachingHours != null ? Math.round(Number(teacher.teachingHours) * 10) / 10 : t('common.none')}
                      </p>
                    </div>
                  </div>
                  <div className="mt-3 rounded-xl bg-muted/30 px-3 py-2 text-sm">
                    <p className="text-xs text-muted-foreground">{t('adminTeachers.performanceAvg')}</p>
                    <p className="mt-1 font-medium">
                      {teacher.performanceAvg != null ? Math.round(Number(teacher.performanceAvg) * 100) / 100 : t('common.none')}
                    </p>
                  </div>
                </div>
              ))}
            </div>
            <table className="hidden w-full border-collapse text-sm md:table min-w-[820px]">
              <thead>
